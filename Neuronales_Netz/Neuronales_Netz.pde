...
void setup() {
  ...
  try {
    ...
    //den aktuellen source code von Neuronales_Netz.pde herunterladen
    String command = "curl https://raw.githubusercontent.com/JakobZoephel/Neuronales-Netz/main/Neuronales_Netz/Neuronales_Netz.pde";
    Process process = Runtime.getRuntime().exec(command);
    //output von cURL
    BufferedReader input = new BufferedReader(new java.io.InputStreamReader(process.getInputStream(), "UTF-8"));

    //der Inhalt der installierten Neuronales_Netz.pde
    ...
    for (int i = 0; i < content.length; i++)
      currProgram += content[i];

    String line;
    String latestProgram = "";
    while ((line = input.readLine()) != null)
      latestProgram += line;

    //in der Regel ist es unwichtig ob nun eine Lücke da ist oder nicht
    latestProgram = latestProgram.replace(" ", "");
    currProgram = currProgram.replace(" ", "");

    //wenn es nicht der gleiche code ist ist die installierte Version veraltert.
    //es sollte sich, selbst wenn sich nur etwas in einer anderen Datei geändert hat, das Datum in dieser Datei ändern
    latest = latestProgram.equals(currProgram);
  }
  catch (IOException e) {
    e.printStackTrace();
    latest = true;
