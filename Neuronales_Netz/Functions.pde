...
class Functions {
  ...
  public float[] softmax(float[] outputs) {
    ...
    if (mousePressed)printArray(outputs);
    return outputs;
  }
  ...
  // filter nicht numeric types raus
  <T> ArrayList<T> filterNumerics(T[] t) {

    ArrayList<T> checkedT = new ArrayList<T>(t.length);

    for (int i = 0; i < t.length; i++)
      checkedT.add(t[i]);

    for (int i = checkedT.size()-1; i >= 0; i--) {

      try {
        // ob es ein float ist
        Float.parseFloat(checkedT.get(i).toString());
      }
  ...
  int savedExamples() {
    ...
    // wenn es 0 Beispiele sind
    if (examples == null)
      return 0;
    return examples.length/(trainExampleData/7);
  }

  int max_pool_array(float[] array) {
    float a = max(array);
