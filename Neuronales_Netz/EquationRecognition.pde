class EquationRecognition {
  ...
  void slowRecognition(PImage picture) {
    ...
    consoleText.clear();
    visualConsole.consoleIndex = 0;
    consoleText.add("normalizing image...");
    picture = normalizeImage(picture);
    int thickness = F.thickness(picture);

    if (thickness < drawSize)
      picture = F.addThickness(drawSize-thickness, picture);
    println("trying to detect...");
    ...
    for (int i = 0; i < picture.pixels.length; i++) {
      ...
      if (picture.pixels[i] == color(0)) {
        ...
        // keine Zahl sondern nur ein einzelner Pixel
        if (extractedImage.width < drawSize*2 && extractedImage.height < drawSize*2)continue;
  ...
  void recognize(ArrayList<ArrayList<PImage>> pictureList, ArrayList<PVector> equalSigns) {
    ...
    for (int currRow = 0; currRow < pictureList.size(); currRow++) {
      for (int i = 0; i < pictureList.get(currRow).size(); i++) {
        pictureList.get(currRow).set(i, F.addThickness(drawSize, pictureList.get(currRow).get(i)));
        ...
        //falls die kernel_size dazu führt, dass das Bild nicht 28*28 ist
        pictureList.get(currRow).get(i).resize(28, 28);
        if (currRow == 1 && i == 2)
          debuggPic =  pictureList.get(currRow).get(i).get();
