class NeuronalNetworkVisualization {
  ...
  int offset = 100;
  //welches Netz gezeigt werden soll z.B. das programmierte oder PyTorch
  byte type = 1;
  ...
  void showResult() {
    ...
    if (spacePressed) {
      ...
      text("space pressed - freeze", width/2, height-20 );
      pop();
      return;
    }
    //ob er mit der Animation fertig ist oder nicht
    if (finished) {
      ...
      if (millis() >= countdown+3000 && countdown != 0) {
        ...
        rectMode(CORNER);
        fill(33);
        rect(width/2*type, 0, width/2, 30);
        fill(textColor);
        textAlign(CENTER, CENTER);
        textSize(18);
        text("this AI", width/4, 10);
        text("PyTorch AI", width/2+width/4, 10);

        //type 0 == PyTorch
        if (mousePressed && mouseX > width/2 && mouseX < width && mouseY > 0 && mouseY < 30)
          type = 0;
        //type 1 == eigene AI
        else if (mousePressed && mouseX > 0 && mouseX < width/2 && mouseY > 0 && mouseY < 30)
          type = 1;

        float[] showArray = lm.ergebnis;

        if (type == 0)
          showArray = PyTorchResult;

        if (showArray == null) {
          text("PyTorch konnte nicht gestartet werden", width/2, height/2-100);
        } else {
          int iterator = 0;
          for (int i=0; i < showArray.length; i++) {
            if (showArray[i] == max(showArray)) {
              fill(255);
              textSize(60);
              textAlign(CENTER, CENTER);
              text(charset[i], width/2, height/2-100);
              textSize(15);
              text(str(showArray[i]*100)+'%', width/2, height/2-50);
            } else {
              fill(155);
              textSize(20);
              textAlign(LEFT, CENTER);
              text(charset[i], width/(showArray.length-1)*iterator+10, height/2+100);
              textSize(13);

              String s = str(showArray[i]*100);
              //wenn er zu viele Stellen hat
              if (s.length() > 10)
                s = s.substring(0, s.length()-5) + s.substring(s.length()-4, s.length());

              s += '%';
              text(s, width/(showArray.length-1)*iterator+10, height/2+120);
              iterator++;
            }
          }
        }
